...
rdata
r<-is.na(rdata)
rdata[complete.cases(rdata[,2],rdata[,3])]
rdata1=rdata[complete.cases(rdata[,2],rdata[,3])]
rdata1
...
which(ra==TRUE,arr.ind = T)

install.packages("dplyr")
library(dplyr)
table(is.na(rdata$v2))
table(is.na(rdata$v3))

rdata2<-df %>% filter(!is.na(rdata$v2) & !is.na(rdata$v3))
